...
public class Parque {
    ...
    public Parque(int numPuertas) {
        ...
        for (int i = 0; i < numPuertas; i++) {
            puertas[i] = new Puerta(i + 1); // crea cada puerta con un numero
        }
    }
    ...
    // genera un codigo de acceso unico para cada visitante
    private synchronized int obtenerCodigoAcceso() {
        return codigoAcceso++;
    }

    // intenta acceder al parque por las puertas disponibles
